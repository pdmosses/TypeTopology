...
#
	@for file in $(TEMP)/*; do \
	    ...
	    _='# Ensure the directory for the page exists and move the file to the page:'; \
	    mkdir -p $$(dirname $${page}) && mv -f $${file} $${page};  \
	    \
	    _='# Adjust the pre and code tags, depending on the file type:'; \
	    case $${file} in \
	    *.html) \
	    ...
	    *) \
	    ...
	    _='# Set the relative path from the page to MD:'; \
	    page2md=$$(echo $${relurl} | sd '[^/]*/' '../'); \
	    _='# Prefix local urls by the relative path to MD:'; \
	    sd "href=\"([^:\"][^:\"]*)\"" "href=\"$${page2md}\$$1\"" $${page}; \
	    \
	    _='# If the HTML pages are included in the generated website:'; \
	    if [ -z "$(DOCS2HTML)" ] || [ "$(DOCS2HTML)" != $(HTML)/ ]; then \
		_='# Replace the href and class in the anchor for the module definiton:'; \
		sd "<a id=\"([^\"]+)\" href=\"$${page2md}$${relurl}\" class=\"Module\">" \
		   "<a id=\"$$1\" href=\"$${page2md}$(MD2DOCS)$(DOCS2HTML)$${htmlpagename}.html\" class=\"Module Definition\">" \
	           $${page}; \
	    fi; \
	    \
	done
